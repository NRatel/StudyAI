...
>
> 读本模块时，先抓住 Agent 为什么不只是聊天，以及工具调用、记忆、规划、多 Agent、MCP 分别解决什么问题。框架 API 只作为背景，不是主线目标。

## 本模块导读

**学什么**：从单次问答到自主完成复杂任务——AI Agent 的完整技术栈。从 ReAct 框架的"推理+行动"循环，到 Tool Use 的函数调用机制，到多 Agent 协作与 MCP 协议，理解 Agent 如何感知、推理、规划、行动。

> **常见误解澄清：Agent 不等于多轮对话。** 多轮对话只是用户和模型之间的来回聊天，模型被动地回应每一轮输入，不会主动采取行动。Agent 的核心是**自主决策和工具调用**——Agent 会自主判断下一步做什么、调用什么工具、如何根据工具返回的结果调整策略，整个过程无需用户逐步指导。一个普通的多轮聊天机器人不是 Agent；一个能自主搜索信息、执行代码、调用 API 并根据结果迭代决策的系统才是 Agent。
...
1. Agent 为什么不等于多轮对话？“自主决策”和“工具调用”体现在哪里？
2. ReAct 的“推理 + 行动”循环解决了什么问题？
3. 短期记忆、长期记忆、工作记忆分别适合存什么？
4. 多 Agent 协作什么时候有价值，什么时候只是增加复杂度？
5. MCP 解决的是模型能力问题，还是工具接入协议问题？
