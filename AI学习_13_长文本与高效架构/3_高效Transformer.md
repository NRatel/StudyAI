...
> **一个重要的工程现实**：高效注意力方案（线性注意力、稀疏注意力、SSM 等）在理论上非常优美——O(n) 复杂度、无 KV Cache 膨胀、固定状态推理。但截至 2026 年，在大多数实际 LLM 场景中，**FlashAttention + 标准全注意力**仍然是更实用的选择。原因不在于理论优劣，而在于工程生态：
>
> - FlashAttention 经过数年优化，在主流 GPU 上的实现极其成熟稳定，与所有主流框架（PyTorch、vLLM、TensorRT-LLM）无缝集成
> - 标准注意力的训练和推理行为被充分研究，debug 工具完善，工程团队经验丰富
> - 高效变体的 GPU kernel 实现通常不如 FlashAttention 优化程度高，理论上的复杂度优势在实际硬件上可能被常数因子抵消（特别是在 n < 32K 的常见场景下）
>
> 这不意味着高效注意力没有价值——在超长序列（>128K）、边缘部署、流式推理等特定场景中，Mamba/RWKV 的优势是真实的。但作为工程师，不应因为理论复杂度更低就盲目选择高效变体，而应根据实际场景的序列长度、硬件条件和生态成熟度做务实判断。
