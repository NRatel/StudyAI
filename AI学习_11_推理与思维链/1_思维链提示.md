...
```python
"""
假说可视化：CoT 如何扩展有效计算深度

Transformer 的每层计算可以看作一次"思考"：
- 没有 CoT：所有计算必须在 L 层内完成
- 有 CoT：每个 token 生成都触发 L 层计算，N 个中间 token = N×L 层有效计算
"""

def standard_inference(model, question):
    """标准推理：一步到位"""
    # 模型用 L 层计算直接从问题映射到答案
    # 计算预算 = L 层
    answer = model.generate(question, max_tokens=1)
    return answer

def cot_inference(model, question):
    """CoT 推理：逐步生成"""
    # 模型先生成中间步骤，再生成答案
    # 每个 token 生成都经过 L 层计算
    # 如果生成了 N 个中间 token，有效计算预算 = N × L
    full_response = model.generate(question, max_tokens=256)
    # 从完整响应中提取最终答案
...
def extract_number(text: str) -> str | None:
    """从推理文本中提取最终数字答案"""
    # 尝试匹配 "答案是 X" 或 "the answer is X" 等模式
    patterns = [
        r'答案[是为：:]\s*(\-?\d+(?:\.\d+)?)',
        r'[Tt]he answer is\s*(\-?\d+(?:\.\d+)?)',
        r'[Tt]herefore.*?(\-?\d+(?:\.\d+)?)\s*[。.]?\s*$',
        r'=\s*(\-?\d+(?:\.\d+)?)\s*[。.]?\s*$',
    ]
...
def self_consistency(
    ...
    for i in range(n_samples):
        ...
        answer = extract_number(reasoning)
        if answer is not None:
            answers.append(answer)

    # 多数投票
    if not answers:
        return {"answer": None, "confidence": 0, "paths": reasoning_paths}
...
# ----- 使用示例 -----
question = """
一个水池有两个水管。进水管每小时进 120 升水，
出水管每小时排 80 升水。水池容量是 800 升。
两个管同时打开，多少小时能把空水池装满？
"""

result = self_consistency(question, n_samples=10, temperature=0.7)
...
```python
"""
成本-收益分析：Self-Consistency 的采样次数
"""

# 模拟 Self-Consistency 随采样次数的收益曲线
# 基于 Wang 2023 论文中 GSM8K 的实际数据
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt

n_samples = [1, 5, 10, 15, 20, 30, 40]
accuracy = [56.5, 67.2, 71.0, 72.8, 73.5, 74.1, 74.4]
cost_multiplier = n_samples  # 成本与采样次数成正比

# 边际收益 = 每多 1 次采样带来的准确率提升
marginal_gain = [0] + [
    (accuracy[i] - accuracy[i-1]) / (n_samples[i] - n_samples[i-1])
    for i in range(1, len(n_samples))
]

print("采样次数 | 准确率 | 成本倍数 | 边际收益（每次采样）")
print("-" * 55)
for i in range(len(n_samples)):
    print(f"  {n_samples[i]:>3}    | {accuracy[i]:>5.1f}% | {cost_multiplier[i]:>4}x   | {marginal_gain[i]:>+.2f}%")

# 输出:
# 采样次数 | 准确率 | 成本倍数 | 边际收益（每次采样）
# -------------------------------------------------------
#     1    |  56.5% |    1x   | +0.00%
#     5    |  67.2% |    5x   | +2.68%
#    10    |  71.0% |   10x   | +0.76%
...
# 结论：5~10 次采样是性价比甜点。超过 20 次后边际收益几乎为零。
```
...
## 六、工程视角：CoT 在生产环境中的使用

### 6.1 什么时候该用 CoT

> **CoT 不是万能的**：对简单任务，CoT 可能引入不必要的推理步骤，反而降低准确率。例如，"法国首都是哪里？"这种单步事实查询，强制使用 CoT 会让模型生成冗余的"推理"过程（"首先，法国是一个欧洲国家...然后..."），不仅浪费 token，还可能在多余的推理步骤中引入错误。Wei et al. (2022) 的原始论文也明确指出：CoT 在简单任务上几乎无提升（PaLM 8B 上甚至略有下降），只在需要多步推理的复杂任务上才有显著收益。**使用 CoT 之前，先判断任务是否真的需要多步推理。**
...
**不适合 CoT 的场景**：
- 简单的事实查询（"法国首都是哪里？"——CoT 反而增加出错概率）
- 翻译（逐字翻译不如直接翻译）
- 创意写作（不需要严格的逻辑链）
- 延迟敏感的场景（CoT 增加生成 token 数，增加延迟）
...
# 实践 2：答案提取要鲁棒
def robust_answer_extraction(response: str, answer_type: str = "number") -> str:
    """
    从 CoT 响应中鲁棒地提取最终答案
    ...
    if answer_type == "number":
        # 先找显式的"答案是..."
        import re
        explicit = re.search(r'(?:答案|结论|结果)[是为：:]\s*(\-?\d+(?:\.\d+)?)', response)
        if explicit:
            return explicit.group(1)
        # 兜底：取 "## 结论" 后的第一个数字
        conclusion_match = re.search(r'(?:结论|结果).*?(\-?\d+(?:\.\d+)?)', response, re.DOTALL)
        if conclusion_match:
        ...
        # 选择题：提取 A/B/C/D
        explicit = re.search(r'(?:答案|选)[是为：:]\s*([A-Da-d])', response)
        if explicit:
            return explicit.group(1).upper()
        # 兜底
        choices = re.findall(r'\b([A-D])\b', response)
        return choices[-1] if choices else None

# 实践 3：CoT + Self-Consistency 的工程化封装
class ReasoningEngine:
    ...
    def reason(
        self,
        question: str,
        strategy: str = "zero_shot_cot",  # "direct" | "zero_shot_cot" | "self_consistency"
        n_samples: int = 5,
        temperature: float = 0.7
    ) -> dict:
        """
        ...
        根据任务难度和预算选择策略：
        - direct: 简单问题，直接回答
        - zero_shot_cot: 中等难度，单次 CoT
        - self_consistency: 高难度 / 高准确率要求
        """
        if strategy == "direct":
            ...
            return self._self_consistency(question, n_samples, temperature)
        else:
    ...
    def _zero_shot_cot(self, question):
        resp = self.client.chat.completions.create(
            model=self.model,
            messages=[{
                "role": "user",
        ...
        answer = robust_answer_extraction(full)
        return {
            "answer": answer,
            "reasoning": full,
    ...
    def _self_consistency(self, question, n_samples, temperature):
        answers = []
        paths = []
        for _ in range(n_samples):
            ...
            full = resp.choices[0].message.content
            paths.append(full)
            ans = robust_answer_extraction(full)
            if ans:
                answers.append(ans)
...
| 陷阱 | 现象 | 解决方案 |
|------|------|----------|
| 推理正确但答案提取失败 | 模型完美推理出 19，但最终说"答案是 20" | 使用结构化模板 + 鲁棒提取 |
| 过度推理 | 简单问题被过度分析，反而出错 | 对简单问题不使用 CoT |
| 幻觉推理 | 模型编造看似合理但错误的推理步骤 | 使用 Self-Consistency 过滤 |
| 小模型乱推 | <10B 模型生成错误推理链 | 换更大模型或放弃 CoT |
| 成本爆炸 | Self-Consistency 40 次采样太贵 | 5~10 次采样通常足够 |
