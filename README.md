...
![AI 及大模型原理系统学习封面](assets/ai-study-cover.png)

## 内容特点

- **GPT-5.5 生成正文，先把话讲清楚**：每节优先回答“是什么、为什么出现、解决什么问题、和前后技术有什么关系”，不一上来堆公式和论文细节。
- **GPT-Image-2 生成配图，让抽象概念有画面**：配图同时保留专业结构和生动类比，把神经网络、注意力、扩散、RAG、Agent 这类抽象机制画成更容易记住的流程。
- **完整 13 章主线，不是术语拼盘**：从神经网络基础一路串到 Transformer、GPT、生成模型、训练推理、RAG、多模态、推理模型、Agent 和长文本架构。
- **适合建立判断力**：读完后不只是“听过这些词”，而是能判断一个概念大概属于哪一层、解决什么问题、常见说法哪里可能不严谨。
- **轻量但不浅薄**：主线尽量顺着直觉走，必要的数学、代码和工程细节放在附录或后续章节，需要时再深入，不打断第一次阅读。
- **适合连续阅读**：README 适合快速了解全貌，HTML 版本提供完整目录、章节跳转和配图，更适合一章一章读下去。

这是一份面向有编程经验、但 AI 原理基础不多的读者的概念学习材料。它的重点不是训练你做论文复现或工程调参，而是帮你把 AI 与大模型的主线串起来：神经网络如何学习，Transformer 为什么重要，GPT 为什么能生成内容，RAG、Agent、多模态和推理模型又分别在补什么能力。

读完后，你应该能更稳地看懂 AI 相关术语、技术脉络和常见讨论，不会一听到 Attention、Scaling Laws、RLHF、KV Cache、Embedding、CoT、Agent 就只能点头。

## 这份内容解决什么

- **概念定义**：优先讲清“是什么”，避免只留下模糊印象。
- **机制直觉**：用日常类比、图示和简化解释说明“为什么这样设计”。
- **技术关系**：把概念放回上下游脉络里，看清它解决的问题和留下的问题。
- **常见误区**：提醒哪些说法容易听起来对、实际不严谨。
- **按需深入**：数学、代码和工程细节放在附录或后续章节中，不抢主线。
...
- 完全没有编程基础，且暂时不打算补基础的人。
- 主要目标是刷项目、练部署、调框架参数的人。
- 需要严格数学证明、论文级细节或可发表研究综述的人。
