...
> 读本模块时，先抓住生成模型的谱系，以及扩散模型“先加噪、再学会去噪”的核心直觉。复杂概率推导和采样细节可以按需查阅。
...
1. VAE、GAN、Flow、Diffusion 都是在“生成数据”，但它们的基本思路有什么不同？
2. 扩散模型的前向加噪和反向去噪分别在做什么？
3. DDPM 的训练目标为什么常被写成“预测噪声”？
4. Classifier-Free Guidance 为什么能增强条件控制？
5. Stable Diffusion 为什么要在潜空间里扩散，而不是直接在像素空间里扩散？
