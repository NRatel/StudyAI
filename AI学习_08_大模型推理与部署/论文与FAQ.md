...
> **建议先读 Kwon 2023 (vLLM/PagedAttention)**。PagedAttention 是理解现代推理优化的关键——它解释了为什么 KV Cache 的显存管理是推理吞吐的核心瓶颈，以及如何用操作系统的分页思想来解决。理解了这篇论文，后续的量化（为什么要减少模型体积）和推测解码（为什么要减少串行步数）的动机就一目了然。
