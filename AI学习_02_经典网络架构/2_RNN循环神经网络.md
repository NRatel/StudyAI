...
> **收尾总结**：尽管 Transformer 已在主流 NLP 任务中全面取代了 RNN，但 RNN 在边缘设备部署（参数少、推理内存恒定）、流式处理（逐步输入逐步输出）以及超长序列的低内存场景中仍有实际价值。学习 RNN 既是为了理解序列建模的基本思想，也是为了明白 Transformer 究竟解决了什么问题。
