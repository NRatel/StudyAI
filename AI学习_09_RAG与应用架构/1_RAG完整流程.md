...
> **排查要点**：RAG 系统的失败分两大类，排查时必须分开诊断：
> 1. **检索失败**（找不到相关内容）：表现为回答"不知道"或产生幻觉。根因在 Embedding 模型、Chunking 策略或查询本身。解法是改善检索环节（Query 改写、HyDE、混合检索等）。
> 2. **生成失败**（找到了但答错）：检索结果中包含正确信息，但 LLM 没有正确使用。根因在 Prompt 设计、上下文过长/过多噪声、或 LLM 能力不足。解法是优化 Prompt、加 Reranker 精排、换更强模型。
>
> **诊断方法**：先检查 Top-K 检索结果中是否包含回答所需信息。如果包含，问题在生成侧；如果不包含，问题在检索侧。
