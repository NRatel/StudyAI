...
results = collection.search(
    ...
    limit=10,
    # 关键：元数据过滤
    expr=f'access_level in ["public", "internal"] and department == "{user.department}"',
)
```
...
LangChain 官方的 RAG 教程，从最基础的 "stuff" 链到高级的多步 RAG，配有完整代码和解释。适合从零开始动手构建 RAG 应用。涵盖 Document Loader、Text Splitter、Vector Store、Retriever、Chain 的完整使用方法。
...
介绍了生产级 RAG 系统的架构设计、常见陷阱和最佳实践。涵盖 Chunking 策略优化、检索质量提升、评估体系建立等工程话题。对于正在构建或优化 RAG 系统的工程师非常实用。
