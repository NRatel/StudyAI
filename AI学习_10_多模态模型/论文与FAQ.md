...
在深入逐篇论文之前，先把握本模块的论文主线和代表性应用：

```
主线论文（奠基性工作，必读）：
  ViT (2020) → CLIP (2021) → LLaVA (2023)
  │              │              │
  │              │              └── 将 CLIP 视觉编码器接入 LLM，开创开源多模态范式
  │              └── 用对比学习建立图文共享空间，奠定多模态桥接基础
  └── 将 Transformer 引入视觉领域，取代 CNN

代表性应用（各领域标志性工作）：
  Whisper (2022) ── 语音识别领域的"CLIP 时刻"，68 万小时弱监督数据
  Sora (2024) ── 视频生成领域的里程碑，DiT + 时空扩散
```

理解主线论文是理解整个多模态技术栈的关键：ViT 解决了"图像如何进入 Transformer"，CLIP 解决了"图文如何对齐"，LLaVA 解决了"视觉编码器如何接入 LLM"。三者构成了开源多模态的完整技术链条。
